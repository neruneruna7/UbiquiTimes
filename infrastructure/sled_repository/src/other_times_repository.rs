...
impl SledOtherTimesRepository {
    ...
    fn fillter_member_id(&self, member_id: u64) -> SledOtherTimesRepositoryResult<Vec<OtherTimes>> {
    ...
    fn fillted_data(
        ...
        server_name: &str,
        member_id: u64,
    ) -> SledOtherTimesRepositoryResult<Vec<OtherTimes>> {
        let other_times_table = OtherTimesTable::new(&self.db);
        let data = other_times_table.read_all()?;
        // member_idが一致するものを抽出
        let fillter_data: Vec<OtherTimes> = data
            ...
            .filter(|x| {
                x.other_times_data.src_member_id == member_id
                    && x.other_times_data.dst_guild_name == server_name
            })
            .map(|x| x.other_times_data)
            .collect();
        Ok(fillter_data)
